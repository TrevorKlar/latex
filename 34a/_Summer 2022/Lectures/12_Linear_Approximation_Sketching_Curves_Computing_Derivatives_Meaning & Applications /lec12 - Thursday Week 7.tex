...
\frame{
 \frametitle{Why use $h$ to find the derivative?}
   {\large\blue$\text{Without }h\text{: \ }\fbox{f'(a) = \lim\limits_{x \rightarrow a} \frac{f(x)-f(a)}{x-a}}$}
   
   Here is an example without $h$. For $f(x) = x^2$, if we wanted to find $f'(2)$ it would be the limit of the average rate of change from 2 to a second point $x$ as that second point approaches 2. \pause
   $$\underset{x\rightarrow2}{\lim} \frac{x^2 - 2^2}{x-2}\pause=\underset{x\rightarrow2}{\lim} \frac{(x - 2)(x+2)}{(x-2)}\pause=\underset{x\rightarrow2}{\lim} \ x + 2\pause=4$$\pause
   Second example: For $g(x) = x^3$, if we wanted to find $g'(5)$ it would be the limit of the average rate of change from 5 to a second point $x$ as that second point approaches 5. \pause
   $$\underset{x\rightarrow5}{\lim} \frac{x^3 - 5^3}{x-5}\pause=\underset{x\rightarrow5}{\lim} \frac{(x - 5)(x^2+5x + 5^2)}{(x-5)}\pause=\underset{x\rightarrow5}{\lim} \ x^2+5x + 5^2\pause=75$$ \pause
...
\frame{
  ...
  \begin{center}
    \includegraphics[height=.8\textheight]{Lecture 12 Picture.jpg}
  \end{center}
}
