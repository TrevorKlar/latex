...
\begin{document}
\begin{center}
\textbf{{\LARGE MATH 3B}  \hfill} Discussion Worksheet - Thursday, May 17\\
...
\item When do we use PFD?: %to integrate rational functions (which look like polys/polys)
...
\item Strategy: Given a rational function of the form $p(x)/q(x)$ there are several cases to consider.


\vspace{1.5in}
%(1) Rule 1: deg(p) < deg(q) ALWAYS If it isn't, polynomial long division to get a new function + remainder in the desired form
...
\item How do we ensure Rule 1? We long divide polynomials, just like we did long division in 4th grade.

Example: Rewrite $\dfrac{x^3 - 2x^2 - 4}{x^3 - 2x^2}$ so that it is written as a sum of rational functions with the degree of the numerator less than the degree of the denominator.

\newpage

\item Example: Find $\ds \int \dfrac{x^3 - 2x^2 - 4}{x^3 - 2x^2} \, dx$ using your long division.

\vspace{4in}

\item Set Up the PFD for the following Special Cases:

(1)  $\ds \int \dfrac{x^4 + 1}{x^5-x^3} \, dx$ \hspace{0.5in} (2) $\ds \int \dfrac{x^4 + 1}{x^5-x^3} \, dx$  \hspace{0.5in} (3) $\dfrac{x^3 + x^2 + 1}{x(x-1)(x^2+x+1)(x^2+1)^3}$
