...
\item General Antiderivatives: %an antiderivative of a function f(x) is a function F(x) such that the derivative of F(x) is f(x)
...
%\newpage
%
%\noindent\textbf{Definitions and Examples Continued:}
%
%\begin{itemize}
%
%\item Fundamental Theorem of Calculus Part 1: If $g(x) = \ds \int_{a}^x f(t) \, dt$ then %$g'(x) = f(x)$. 
%
%\item Fundamental Theorem of Calculus Part 2: If $F$ is an antiderivative of $f$, then $$\hspace{-1in}\ds \int_a^b f(x) \, dx = $$% F(a) - F(b)
%
%\end{itemize}
%
%\noindent\textbf{Groupwork and Practice Problems:}
%
%\medskip
%
%\noindent\textbf{Problem 1:} (HW 14) Given $f(x) = \ds \int_0^x (t^3 + 2t^2 + 1) \, dt$, find $f''$. 
%
%\vspace{1in}
%
%\noindent\textbf{Problem 2:} (HW 15) Given $\ds f(x) = \int_0^x \dfrac{t^2 - 49}{t+\cos^2(t)} \, dt$, at what value of $x$ does the local max of $f(x)$ occur?
%
%\vspace{1.15in}
%
%\noindent\textbf{Problem 3:} (HW 16) Suppose $ F(x) =\ds \int_1^x f(t) \, dt,$ where $\ds f(t) = \int_1^{t^3} \dfrac{\sqrt{3+u^6}}{u} \, du$. Find $F''(2).$
%
%\vspace{1.15in}
%
%\noindent\textbf{Problem 4:} (HW 8) The rectangles in the graph give a left endpoint Riemann sum for $f(x) = \dfrac{x^2}{9}$ on $[3,7]$. Compute the left endpoint Riemann sum. Is it an over/underestimate?
%
%\medskip
%
%\noindent\includegraphics[scale=0.4]{Oct3pic}
