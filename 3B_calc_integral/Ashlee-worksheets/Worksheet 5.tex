...
\item[$\cdot$] Force: $F = $ %ma where m is the mass and a is the acceleration (due to gravity)

\bigskip

-Note: %if you are given a \emph{weight}, that IS a force, you don't need to multiply by acceleration (or g) unlike a "mass" of the object

\bigskip\bigskip

\item[$\cdot$] Work: $W = $ % integral of force over some distance (usually force changes as a function of distance)

\bigskip\bigskip

\item[$\cdot$] Work to Move a Particle: %Easiest kind of work problem because Force is given by a function, and you simply integrate the function over a specified time (limits)
...
\item[$\cdot$] Formula: $\ds \int U \, dV =$

\medskip

\item[$\cdot$] Strategies:

\vspace{1.5in}



You Try! For each of the following problems: (1) Determine if it is a IBP problem and if it is, (2) Identify $U$ and $dV$:

(1) $\ds \int \cos(x)\sin^2(x) dx$ \hspace{1in} (2) $\ds \int x^2\ln(x) dx$ \hspace{1in} (3) $\ds \int \arctan(4x) dx$
